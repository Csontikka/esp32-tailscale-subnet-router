...
/* Local extensions (reservation lookup, active-lease enumeration) are
 * declared in "dhcps_ext.h" instead of here, so that callers can pick
 * them up without their <dhcpserver/dhcpserver.h> include accidentally
 * resolving to the IDF-shipped stock header. */
