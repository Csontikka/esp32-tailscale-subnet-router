 ...
 *
 * Plain TCP REPL on a configurable port; the listener stays closed
 * until a non-empty web password is set, then guards every connection
 * with verify_web_password before dropping into esp_console.
 *
...
#define REMOTE_CONSOLE_NVS_NAMESPACE "tsr"

/* Look up an interface's current IPv4 by ESP-NETIF if-key. Returns 0
 * (i.e. "no address") when the netif isn't present or hasn't received
 * an address yet. Used at connection-accept time to figure out which
 * bind-filter group the incoming socket should be charged against. */
static uint32_t netif_ip_by_key(const char *if_key)
{
    esp_netif_t *nif = esp_netif_get_handle_from_ifkey(if_key);
    if (!nif) return 0;
    esp_netif_ip_info_t info;
    if (esp_netif_get_ip_info(nif, &info) != ESP_OK) return 0;
    return info.ip.addr;
}
...
static esp_err_t load_config(void) {
    nvs_handle_t nvs;
    esp_err_t err = nvs_open(REMOTE_CONSOLE_NVS_NAMESPACE, NVS_READONLY, &nvs);
    if (err != ESP_OK) {
        return err;
    }
...
static esp_err_t save_config(void) {
    nvs_handle_t nvs;
    esp_err_t err = nvs_open(REMOTE_CONSOLE_NVS_NAMESPACE, NVS_READWRITE, &nvs);
    if (err != ESP_OK) {
        return err;
    }
...
static void remote_console_task(void *arg) {
    ...
    while (!rc_state.shutdown_requested) {
        ...
        while (!rc_state.shutdown_requested) {
            ...
            /* Tailnet-originated connections (CGNAT 100.64.0.0/10) are
             * accepted only when the user explicitly opted in by
             * checking the "Tailscale" bind box in /config. Default OFF
             * keeps the safer "LAN-only CLI" posture, but the option
             * exists for setups where Tailnet ACLs do the gating. */
            {
                uint32_t client_ip_hbo = ntohl(client_addr.sin_addr.s_addr);
                bool from_tailnet = ((client_ip_hbo & 0xFFC00000u) == 0x64400000u);
                if (from_tailnet && !(rc_config.bind & RC_BIND_VPN)) {
                    ESP_LOGW(TAG, "Connection from %s rejected (Tailnet CGNAT — "
                             "enable 'Tailscale' bind in /config to allow)",
                             rc_state.client_ip);
            ...
            /* Check interface binding - reject connections on disallowed interfaces */
            {
                ...
                uint32_t ap_ip  = netif_ip_by_key("WIFI_AP_DEF");
                uint32_t sta_ip = netif_ip_by_key("WIFI_STA_DEF");
                bool allowed = false;
                if ((rc_config.bind & RC_BIND_AP)  && ap_ip  && local_ip == ap_ip)  allowed = true;
                if ((rc_config.bind & RC_BIND_STA) && sta_ip && local_ip == sta_ip) allowed = true;
                if ((rc_config.bind & RC_BIND_VPN) && from_tailnet)                 allowed = true;
