...
#define SYSLOG_NVS_NAMESPACE "tsr"

/* STA link state owned by main.c. We won't push syslog packets while the
 * uplink is down — they'd just queue up and waste heap. */
...
/* Cached device hostname — read once at load_config() from the shared
 * NVS "hostname" key written by /api/network. Falls back to "ESP32" if
 * unset. Length matches what the network UI accepts. */
...
{
    nvs_handle_t nvs;
    if (nvs_open(SYSLOG_NVS_NAMESPACE, NVS_READONLY, &nvs) != ESP_OK)
        return;
    ...
    size_t hlen = sizeof(s_hostname);
    nvs_get_str(nvs, "hostname", s_hostname, &hlen);
...
{
    nvs_handle_t nvs;
    esp_err_t err = nvs_open(SYSLOG_NVS_NAMESPACE, NVS_READWRITE, &nvs);
        ...
        if (s_rawbuf && s_pktbuf && atomic_load(&s_resolved)) {
            ...
            if (has_newline) {
                ...
                if (clean_len > 0 && !is_stub) {
                    ...
                    const char *host = s_hostname[0] ? s_hostname : "ESP32";
...
static void start_sender(void)
{
    ...
    if (!s_sender_task && s_queue) {
        /* 5120 matches the exit_route / ping stack budget used after a
         * vApplicationStackOverflowHook PANIC under busier tailnet load:
         * sender_task does socket I/O + ESP_LOG (256B vsnprintf scratch)
         * + LwIP DNS lookups, so 3072 is a known underrun. */
        xTaskCreate(sender_task, "syslog_tx", 5120, NULL, 5, &s_sender_task);
    }
}
...
void syslog_notify_connected(void)
{
    if (atomic_load(&s_enabled) && !atomic_load(&s_resolved)) {
        xTaskCreate(syslog_connect_task, "syslog_dns", 5120, NULL, 5, NULL);
    }
}
